...
test('isSameDay should return true for same days', () => {
    ...
    assert.equal(Utils.isSameDay(new Date(2022, 9, 23), new Date(2022, 9, 23, 15, 10, 0)), true);
});

test('isSameDay should return false for different days', () => {
    assert.equal(Utils.isSameDay(new Date(2022, 10, 23), new Date(2022, 9, 23)), false);
});

test('parseDate should parse date from string', () => {
    assert.deepEqual(
        Utils.parseDate('https://www.mujkaktus.cz/api/download?docUrl=%2Fapi%2Fdocuments%2Ffile%2FOP-Odmena-za-dobiti-FB_23062025.pdf&filename=OP-Odmena-za-dobiti-FB_23062025.pdf'),
        new Validity(new Date(2025, 5, 23)),
    );
});

test('parseDate should not parse date from string', () => {
    assert.equal(Utils.parseDate('https://sluzby.mujkaktus.cz/moje-sluzby'), null);
});
